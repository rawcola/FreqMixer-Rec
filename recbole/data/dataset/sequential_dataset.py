...
class SequentialDataset(Dataset):
    ...
    def data_augmentation(self):
        ...
        for i, uid in enumerate(self.inter_feat[self.uid_field].numpy()):
            if last_uid != uid:
                ...
                if i - seq_start > max_item_list_len:
                    seq_start += 1
                elif i - seq_start >= self.config['MIN_ITEM_LIST_LENGTH']:
                    uid_list.append(uid)
        ...
        print("target_index size:", target_index.shape)
        print("item_list_index:", item_list_index.shape)
        ...
        new_dict = {
            ...
            'target_index': torch.tensor(self.target_index)
        }

        for field in self.inter_feat:
            if field != self.uid_field:
                list_field = getattr(self, f'{field}_list_field')
#                if not list_field == "item_id_list":
#                    continue
...
# user_id,           item_id,      item_id_list
#[u1 u1, u2, u2]    [i1,i2,i1,i3]    [[i_x, i_y], [xx], [xx], [xx]]
#
