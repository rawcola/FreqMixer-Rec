...
class Trainer(AbstractTrainer):
    ...
    def __init__(self, config, model):
        ...
        self.knn_negative_samples = None
        self.knn_negative_item_samples = None
    ...
    def _build_lr_scheduler(self):
        return lr_scheduler.StepLR(self.optimizer, step_size=25, gamma=0.1)
        # if self.config['dataset'] == 'ml-1m':
        #     return lr_scheduler.StepLR(self.optimizer, step_size=20, gamma=0.1)
        # elif self.config['dataset'] == 'beauty':
        #     return lr_scheduler.StepLR(self.optimizer, step_size=9, gamma=0.1)
        # elif self.config['dataset'] == 'sport':
        #     return lr_scheduler.StepLR(self.optimizer, step_size=9, gamma=0.1)
        # elif self.config['dataset'] == 'toys':
        #     return lr_scheduler.StepLR(self.optimizer, step_size=9, gamma=0.1)
        # elif self.config['dataset'] == 'yelp2019':
        #     return lr_scheduler.MultiStepLR(self.optimizer, milestones=[5, 7], gamma=0.1)
        # else:
        #     return lr_scheduler.StepLR(self.optimizer, step_size=20, gamma=0.1)
        # return lr_scheduler.CosineAnnealingWarmRestarts(optimizer=self.optimizer, T_0=25, T_mult=2, eta_min=0.00005)
        # return lr_scheduler.CosineAnnealingLR(optimizer=self.optimizer, T_max=30, eta_min=0.00001, last_epoch=-1)
        # return lr_scheduler.MultiStepLR(self.optimizer, milestones=[5, 7], gamma=0.1)
    ...
    def _train_epoch(self, train_data, epoch_idx, loss_func=None, show_progress=False):
        ...
        for batch_idx, interaction in enumerate(iter_data):
            ...
            # neg_samples = None if epoch_idx < 1 else torch.LongTensor(self.knn_negative_samples[batch_idx])
            # knn_items_sample = None if epoch_idx < 1 else torch.LongTensor(self.knn_negative_item_samples[batch_idx])
            # neg_samples = None
            # knn_items_sample = None
            # losses = loss_func(interaction=interaction, knn_negative_samples=neg_samples,
            #                    knn_items_sample=knn_items_sample)
            losses = loss_func(interaction=interaction)
    ...
    def fit(self, train_data, valid_data=None, verbose=True, saved=True, show_progress=False, callback_fn=None):
        ...
        for epoch_idx in range(self.start_epoch, self.epochs):
            ...
            # knn negative sampling
            # self.knn_negative_sampling(train_data, epoch_idx, k=100)
            # self.knn_negative_sampling_items_id(train_data, epoch_idx, k=1)

        self._add_hparam_to_tensorboard(self.best_valid_score)
        return self.best_valid_score, self.best_valid_result

    def _erase_pos_items_id(self, ids, pos_items_id):
        ids_ = []
        k = ids.shape[1] - 1
        for i in range(len(ids)):
            tmp_set = set(ids[i])
            if pos_items_id[i] in tmp_set:
                tmp_set.remove(pos_items_id[i])
            tmp_list = list(tmp_set)[0:k]
            ids_.append(tmp_list)
        return np.array(ids_)

    @torch.no_grad()
    def knn_negative_sampling(self, train_data, epoch_idx, k=1, show_progress=True):
        # knn negative sampling only for sasrec/sascts
        knn_negative_item_ids = {}
        checkpoint_file = self.saved_model_file
        checkpoint = torch.load(checkpoint_file)
        tmp_best_model = copy.deepcopy(self.model)
        tmp_best_model.load_state_dict(checkpoint['state_dict'])
        tmp_best_model.load_other_parameter(checkpoint.get('other_parameter'))
        message_output = 'Loading model structure and parameters from {} for knn negative sampling'.format(checkpoint_file)
        self.logger.info(message_output)
        tmp_best_model.eval()
        item_embedding = tmp_best_model.get_item_embedding_weight()
        embedding_faiss = faiss.IndexFlatIP(item_embedding.shape[1])
        embedding_faiss.add(item_embedding)
        iter_data = (
            tqdm(
                train_data,
                total=len(train_data),
                ncols=100,
                desc=set_color(f"Knn negative sampling item for seq{epoch_idx:>5}", 'pink'),
            ) if show_progress else train_data
        )
        for batch_idx, interaction in enumerate(iter_data):
            interaction = interaction.to(self.device)
            seq_output = tmp_best_model.calculate_seq_output(interaction)
            pos_items_id = tmp_best_model.get_pos_items_id(interaction)
            _, ids = embedding_faiss.search(seq_output, k+1)
            ids = self._erase_pos_items_id(ids, pos_items_id)
            knn_negative_item_ids[batch_idx] = torch.LongTensor(ids)
        self.knn_negative_samples = knn_negative_item_ids

    @torch.no_grad()
    def knn_negative_sampling_items_id(self, train_data, epoch_idx, k=1, show_progress=True):
        # knn negative sampling only for sasrec/sascts
        knn_negative_item_embedding_ids = {}
        checkpoint_file = self.saved_model_file
        checkpoint = torch.load(checkpoint_file)
        tmp_best_model = copy.deepcopy(self.model)
        tmp_best_model.load_state_dict(checkpoint['state_dict'])
        ...
        iter_data = (
            tqdm(
                ...
                total=len(train_data),
                ncols=100,
                desc=set_color(f"Knn negative sampling item for item{epoch_idx:>5}", 'pink'),
            ) if show_progress else train_data
        )
        for batch_idx, interaction in enumerate(iter_data):
            interaction = interaction.to(self.device)
            in_batch_item_emb = tmp_best_model.get_pos_item_embedding(interaction)
            _, ids = embedding_faiss.search(in_batch_item_emb, k+1)
            ids = ids[:, 1:k+1]
            knn_negative_item_embedding_ids[batch_idx] = torch.LongTensor(ids)
        self.knn_negative_item_samples = knn_negative_item_embedding_ids
    ...
    def evaluate(self, eval_data, load_best_model=True, model_file=None, show_progress=False):
        ...
        for batch_idx, batched_data in enumerate(iter_data):
            ...
            if self.gpu_available and show_progress:
                iter_data.set_postfix_str(set_color('GPU RAM: ' + get_gpu_usage(self.device), 'yellow'))
            # batch_item_num = np.array(list(map(self.item_inter_num.get, positive_i.tolist())))
            # scores = scores[batch_item_num < batch_item_num.mean()]
            # interaction = interaction[batch_item_num < batch_item_num.mean()]
            # positive_u = positive_u[batch_item_num < batch_item_num.mean()]
            # positive_i = positive_i[batch_item_num < batch_item_num.mean()]
    ...
    @torch.no_grad()
    def get_mean_output(self, eval_data, load_best_model=True, model_file=None, show_progress=False):
        r"""Evaluate the model based on the eval data.
        ...
        cnt = 0
        output = torch.zeros((64, 50), dtype=torch.float).cuda()
        for batch_idx, batched_data in enumerate(iter_data):
            interaction = batched_data[0]
            cnt += 1
            item_seq = interaction['item_id_list'].cuda()
            tmp_output = self.model(item_seq)
            output += tmp_output.mean(dim=0).transpose(-1, -2)
        output /= cnt
        return output
    ...
    @torch.no_grad()
    def get_attention(self, eval_data, load_best_model=True, model_file=None, show_progress=False):
        if not eval_data:
            return
        ...
        for batch_idx, batched_data in enumerate(iter_data):
            if batch_idx > 0:
                break
            attention = self.model.get_attention(batched_data[0].interaction)

        return attention

    @torch.no_grad()
    def get_seq_representation(self, eval_data, load_best_model=True, model_file=None, show_progress=False):
        if not eval_data:
            return
        ...
        for batch_idx, batched_data in enumerate(iter_data):
            if batch_idx > 0:
                break
            seq_representation = self.model.get_seq_representation(batched_data[0].interaction)
